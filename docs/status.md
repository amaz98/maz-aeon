---
layout: default
title: "Status"
permalink: /status/
---

# Agent Status

**Overall:** 🔴 DEGRADED
**Updated:** 2026-09-22 15:39 UTC
**Open issues:** 1
**Next scheduled run:** heartbeat at 2026-09-22 20:00 UTC

Auto-generated by the `heartbeat` skill on every run (3× daily at 08:00 / 14:00 / 20:00 UTC). If the Updated timestamp is more than ~8h stale, the agent is not running.

## Skill health (last 7 days)

| Skill | Last run | Status | Success rate | Consecutive failures |
|-------|----------|--------|-------------:|---------------------:|
| heartbeat | 2026-09-20 21:16 UTC | ✅ success | 100% | 0 |

## Open issues

| ID | Title | Severity | Category | Detected |
|----|-------|----------|----------|----------|
| [ISS-002](https://github.com/aeonframework/maz-aeon/blob/main/memory/issues/ISS-002.md) | Recurring 08:00 UTC heartbeat dispatch slot skips | high | permanent-limitation | 2026-09-02 |

---
*Fork this repo and your copy inherits this page automatically — [how it works](/memory/).*
